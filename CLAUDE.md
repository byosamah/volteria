...
57. **Device status follows controller status**: Frontend overrides `is_online` to false for all devices when controller is offline (`controller_last_seen > 2 min`). DB cron (`check_device_connection_status`) also marks devices offline in the controller-offline branch. Cloud sync can drain SQLite buffer for 10-15 min after heartbeat stops — without the frontend override, devices appear "online" during this misleading window.
