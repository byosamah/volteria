...
49. **Delta calculated fields have locked frequencies**: DeltaTracker emits completed window totals (not running totals). Frequency locked to 3600s (hourly) / 86400s (daily) — frontend dropdown disabled, DB definitions must match. Cloud downsampling picks FIRST reading per bucket; stable completed totals ensure any sample captures the correct value. Window transitions carry old `latest` as new `first` — consecutive windows are contiguous, no energy lost between polls. **Persistence**: State saved to tmpfs every 60s (survives process restart) and to disk on shutdown at `/opt/volteria/data/delta_tracker_state.json` (survives reboot, max 7-day staleness). Restored on device service startup. **Non-24/7 sites**: Staleness limit is 7 days (not 2h) — sites that shut down overnight preserve state across restarts. As long as 2+ readings exist in a window, the delta is captured. **Meter reset handling**: If counter value decreases (reset to 0), energy splits into segments: (pre-reset delta) + (post-reset delta). Tracked via `accumulated` field in state. Reset at window boundary also handled (fresh start from post-reset value). **Offline devices**: `compute_role_delta()` always includes completed window deltas for all tracked devices — offline devices' last completed total still counted in the sum.
49a. **Initial deployment may leave junk daily delta data**: When calculated fields are first enabled on a new controller, the first cloud sync batch may run before the logging service loads the correct `logging_frequency` from the virtual controller device config. This produces un-downsampled readings (e.g., 5s intervals) for daily fields on the deployment date only. Fix: query cloud, confirm recent days are correct (1/day), DELETE non-midnight entries for the deployment date. Not a code bug — subsequent syncs work correctly.
...
61. **Never log stale readings (3-layer defense)**: (a) `clear_all_readings()` on connection cascade removes all cached values from SharedState. (b) Logging service skips offline devices (`is_online` check). (c) Staleness guard: logging service rejects readings older than 120s by checking each reading's timestamp against current time. This catches ANY future bug. Site calculations also exclude offline devices from computed totals. The logging service re-stamps readings with `current_timestamp` (line 681) for clock alignment — this is by design but means stale cached data looks fresh without these guards.
