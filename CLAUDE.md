...
61. **Connection cascade must clear ALL stale readings**: When a device connection fails, `register_reader.py` cascade only calls `update_reading(success=False)` for the first register that detected the error. Remaining registers are skipped (counter-only). The fix: `clear_all_readings()` is called after cascade to remove ALL stale readings from SharedState. Without this, logging service re-stamps old values with current timestamps → cloud sync uploads → cron marks device online (ghost readings). Defense-in-depth: logging service also skips offline devices.
