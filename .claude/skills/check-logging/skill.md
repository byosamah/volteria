...
- **SQLite freelist pages are expected and non-actionable**: When retention cleanup runs on schedule (1-5 AM daily), freed pages stay on the freelist because `incremental_vacuum` fails silently with active WAL writers. Do not flag freelist as an issue unless disk usage exceeds 80%. The freelist reclaims naturally during next service restart or manual VACUUM.
