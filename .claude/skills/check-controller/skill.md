...
| Serial "Not connected" on ALL registers | `journalctl -u volteria-device` for "Could not exclusively lock port" | Stale FTDI serial lock. Fixed: 3-layer auto-reconnect (close old client, pool eviction, reader trigger). If old code: `sudo systemctl restart volteria-device` |
| First register fails, rest cascade | Check poll order in config registers list | First register hits flaky connection → connection error → cascade skips remaining. Auto-reconnect now evicts stale connection for next poll cycle |
| NaN/Inf value from float32 register | Meter returns invalid float data | Fixed: `modbus_client.py` returns None for NaN/Inf. `site_calculations._get_register_value()` also guards. Prevents NaN propagation into computed sums |
| "Calculations/alarms causing register failure?" | No — they are read-only SharedState consumers | Data flows one direction: Modbus → SharedState → consumers (calculations, alarms, logging). Consumers CANNOT cause register read failures |
