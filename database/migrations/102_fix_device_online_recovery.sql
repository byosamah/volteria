-- Fix: Devices stuck offline after controller comes back online
-- Root cause: Migration 101 set is_online = false in the controller-offline ELSE branch,
-- but recovery (cron + cloud sync lag) takes 5-8+ minutes to restore is_online = true.
-- Solution: Don't modify is_online when controller is offline. Let the frontend
-- controllerOffline override handle the display. DB is_online should only reflect
-- actual device Modbus communication status.
-- Applied: 2026-02-20
...
BEGIN
    ...
    LOOP
        ...
        IF v_controller_online THEN
            ...
            RETURN NEXT;
        ELSE
            -- Controller offline - do NOT modify is_online here.
            -- Frontend controllerOffline override handles the display.
            -- DB is_online should only reflect actual device Modbus status.
